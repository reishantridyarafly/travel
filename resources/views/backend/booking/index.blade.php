...
@section('content')
                                ...
                                <tbody>
                                    ...
                                    @foreach ($transactions as $transaction)
                                        <tr>
                                            ...
                                            <td class="p-3">
                                                <a href="{{ asset('storage/evidences/' . $transaction->photo_evidence) }}"
                                                    target="_blank">
                                                    <img src="{{ asset('storage/evidences/' . $transaction->photo_evidence) }}"
                                                        width="70px" class="img-fluid" alt="image-receipt">
                                                </a>
                                            </td>
                                            ...
                                            <td class="p-3">
                                                {{ date('d/m/Y', strtotime($transaction->booking->start_date)) }} -
                                                {{ date('d/m/Y', strtotime($transaction->booking->end_date)) }}</td>
                                            <td class="p-3">{{ $transaction->name_bank }}</td>
                                            <td class="p-3">Rp {{ number_format($transaction->total, 0, ',', '.') }}
                                            </td>
                                            <td class="p-3">
                                            ...
                                            <td class="p-3">
                                                <a href="javascript:void(0)" class="btn btn-info btn-sm d-flex mb-2"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#detail{{ $transaction->id }}">Detail</a>
                                                @if ($transaction->status == 'pending')
                                                    <form action="{{ route('booking_validated') }}" method="POST"
                                                        class="d-flex mb-2">
                                                        @csrf
                                                    ...
                                                    <form action="{{ route('booking_rejected') }}" method="POST"
                                                        class="d-flex">
                                                        @csrf
                                                        <input type="hidden" name="booking_id"
                                                            value="{{ $transaction->booking_id }}">
    ...
    <!-- Modal Detail -->
    @foreach ($transactions as $transaction)
        <div class="modal fade" id="detail{{ $transaction->id }}" tabindex="-1" aria-labelledby="LoginForm-title"
            aria-hidden="true">
                                ...
                                <div class="container">
                                    <div class="row">
                                        <div class="row">
                                            ...
                                            <div class="col-6 mb-2">
                                                <span
                                                    class="fw-bold">{{ $transaction->booking->package->location }}</span>
                                            </div>
                                            <div class="col-6 mb-2">
                                            ...
                                            <div class="col-6 mb-2">
                                                <span
                                                    class="fw-bold">{{ date('d/m/Y', strtotime($transaction->booking->start_date)) }}</span>
                                            </div>
                                            <div class="col-6 mb-2">
                                            ...
                                            <div class="col-6 mb-2">
                                                <span
                                                    class="fw-bold">{{ date('d/m/Y', strtotime($transaction->booking->end_date)) }}</span>
                                            </div>
                                            @if ($transaction->status == 'success')
                                                ...
                                                <div class="col-6 mb-2">
                                                    <span
                                                        class="fw-bold">{{ date('d/m/Y', strtotime($transaction->updated_at)) }}</span>
                                                </div>
                                            @endif
                                            @if ($transaction->booking->contactDetails && $transaction->booking->contactDetails->isNotEmpty())
                                                <div class="col-6 mb-2">
                                            ...
                                            <div class="col-6 mb-2">
                                                @if ($transaction->photo_evidence != null)
                                                    <td
                                                        class="{{ $transaction->status == 'success' ? 'text-success' : ($transaction->status == 'failed' ? 'text-danger' : 'text-muted') }}">
                                                        {{ strtoupper($transaction->status) }}
                                                    </td>
                                                @else
